...
export default async function DeskPage() {
  ...
  return <PhoneShell>
    ...
    <div style={{ marginTop: 24 }}><Action href="/issue">Issue a new Skin Pass</Action></div>
    <p className="section-label">Active and recent</p>
    <div className="stack">{passes.map((pass) => <Link className="choice" href={`/desk/pass/${pass.id}`} key={pass.id}><div><strong>{pass.client_name}</strong><span>{pass.treatment_name} · {pass.treatment_date}</span></div><span className="choice-status">{pass.status} · V{pass.protocol_version}</span></Link>)}{!passes.length && <div className="disclosure"><h3>No passes yet</h3><p>The first issued pass will appear here with its permanent ID and audit history.</p></div>}</div>
